...
export class BrowserFixtureEventRepository implements EventRepository {
  ...
  private readLocal(): OperationsEvent[] | null {
    ...
    try {
      ...
      return Array.isArray(parsed)
        ? parsed.filter(isOperationsEvent)
        : null;
    } catch {
      return null;
    }
  ...
  async listEvents() {
    return (await this.read()).sort((a,b)=>new Date(a.startsAt).getTime()-new Date(b.startsAt).getTime());
  }
  ...
  async updateEvent(id: string, input: Partial<EventInput> & { status?: EventStatus; cancellationReason?: string }, opts: { now?: Date } = {}) {
    const events = await this.read(); const index = events.findIndex((event) => event.id === id); if (index < 0) throw new Error('Event not found.');
    ...
    await this.write(events); return events[index];
  }
  ...
  async restoreEvent(id: string, status?: EventStatus, opts: { now?: Date } = {}) {
    const event = await this.getEvent(id); if (!event) throw new Error('Event not found.');
    const target = status ?? (event.cancelledAt ? 'cancelled' : event.completedAt ? 'completed' : 'evaluating');
