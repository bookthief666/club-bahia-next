    ...
    <div className="min-h-screen bg-[#050304] text-[#fff6e8]">
      ...
      <div className="relative mx-auto flex min-h-screen w-full max-w-[1480px] flex-col pb-24 md:flex-row md:pb-0">
        <aside className="hidden w-64 shrink-0 border-r border-white/10 bg-black/30 px-5 py-6 backdrop-blur-xl md:block">
          <p className="text-xs uppercase tracking-[.42em] text-amber-200/70">
            Club Bahia
          </p>
            ...
            {nav.map((item) => {
              ...
              return (
                <Link
                  ...
                  className={`block rounded-2xl px-4 py-3 text-sm ${active ? "bg-red-600/25 text-white ring-1 ring-red-300/30" : "text-white/65 hover:bg-white/10 hover:text-white"}`}
                >
                  {item.label}
                </Link>
              );
        ...
        <main className="flex-1 px-4 py-5 min-[390px]:px-5 sm:px-6 lg:px-8">
          <header className="mb-5 flex items-center justify-between gap-4 rounded-3xl border border-white/10 bg-black/35 p-4 shadow-2xl backdrop-blur-xl">
            <div>
              <p className="text-xs uppercase tracking-[.32em] text-amber-200/70">
                Ops console
              </p>
              <h2 className="font-serif text-2xl sm:text-3xl">
                Tonight stays on track.
              </h2>
            </div>
            <div
              className="flex h-12 w-12 items-center justify-center rounded-full border border-amber-200/30 bg-amber-300/10 text-sm font-bold"
        ...
        <nav
          className="fixed inset-x-3 bottom-3 z-20 grid grid-cols-3 gap-2 rounded-3xl border border-white/10 bg-black/80 p-2 text-center text-[11px] shadow-2xl backdrop-blur-xl md:hidden"
        ...
        >
          {nav.map((item) => (
            <Link
              key={item.href}
              href={item.href}
              className="rounded-2xl px-2 py-3 text-white/75 focus:outline-none focus:ring-2 focus:ring-amber-200"
            >
              {item.label}
            </Link>
          ))}
