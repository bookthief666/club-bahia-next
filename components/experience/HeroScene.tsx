...
export function HeroScene() {
  ...
  return (
    <section id="experience-hero" className="relative flex min-h-[100svh] items-start justify-center overflow-hidden px-4 pb-[calc(5.5rem+env(safe-area-inset-bottom))] pt-24 text-center sm:min-h-screen sm:items-center sm:px-6 sm:py-28">
      {/* Future Club Bahia SVG logo / neon crest will replace this temporary CSS ornament. */}
      <motion.div initial="hidden" whileInView="visible" viewport={{ once: true }} className="relative z-10 flex w-full max-w-[min(100%,64rem)] flex-col items-center">
        <motion.div variants={sceneReveal} custom={0.05} animate={reduceMotion ? { opacity: 1, y: 0 } : undefined} className="w-full max-w-[12.5rem] sm:max-w-none">
          <BahiaOrnament />
        </motion.div>
        <motion.p variants={sceneReveal} custom={0.16} className="mt-5 max-w-full text-[0.68rem] uppercase tracking-[0.34em] text-amber-200/80 sm:mt-8 sm:text-xs sm:tracking-[0.45em]">
        ...
        <motion.div variants={sceneReveal} custom={0.34} className="mt-5 flex max-w-full flex-col items-center gap-1 text-[0.62rem] uppercase tracking-[0.16em] text-amber-100/80 sm:mt-8 sm:flex-row sm:gap-5 sm:text-xs sm:tracking-[0.24em]">
