...
export function ExperienceNav({ onOpen }: ExperienceNavProps) {
  ...
  return (
    <header className="fixed inset-x-0 top-0 z-40 px-3 pt-3 sm:px-6 sm:pt-4">
      <nav className="relative mx-auto flex h-12 w-full max-w-7xl items-center justify-between gap-2 overflow-visible border-b border-amber-100/15 bg-[#050304]/70 px-1 pb-2 text-[0.62rem] uppercase tracking-[0.18em] text-amber-100/85 backdrop-blur-md sm:h-auto sm:gap-3 sm:px-0 sm:pb-3 sm:text-[0.66rem] sm:tracking-[0.26em]">
        ...
        <div className="flex min-w-0 items-center justify-end gap-2 sm:hidden">
          {reserveItem && (
            <button
              type="button"
        ...
        {menuOpen && (
          <div id="experience-mobile-menu" className="absolute left-0 right-0 top-full mt-2 grid gap-2 rounded-2xl border border-amber-100/15 bg-[#050304]/95 p-3 shadow-[0_20px_60px_rgba(0,0,0,0.5)] backdrop-blur sm:hidden">
            {menuItems.map((item) => (
              <button
                key={item.label}
                type="button"
